...
namespace OKGamesFramework {
    ...
    /// 中身はSceneDirectorとほぼ同じで、違いはInitAsync関数内のみ.
    ///  - 開発中は作業中のシーンから直起動できるようにする処理が入っている.
    /// </summary>
    public class DebugSceneDirector : MonoBehaviour, ISceneDirector {
        ...
        public async UniTask InitSceneAsync() {
            ...
            var context = sceneContextFactory.CreateGetSceneContext();
        ...
        private async UniTask LoadSceneWithFade(
            ...
            // シーンの1フレーム目はその時のCPU負荷によって重くなることがあるので、1フレ待機してからフェードインする.
